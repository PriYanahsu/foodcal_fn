  ...
  return (
    ...
    >
      <div className="flex gap-4">
        <div className="shrink-0 pt-1">{icons[notification.type]}</div>
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start gap-2">
            <h4
              className={`text-sm font-bold truncate ${notification.isRead ? 'text-gray-400' : 'text-white'}`}
            >
              {notification.title}
            </h4>
            {showRemove && (
          ...
          <p className="text-xs text-gray-300 mt-1 leading-relaxed">{notification.message}</p>
