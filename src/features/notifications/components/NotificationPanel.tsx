...
export const NotificationPanel: React.FC<NotificationPanelProps> = ({ isOpen, onClose }) => {
    const { notifications, unreadCount, markAllAsRead, markAsRead, removeNotification, addNotification, permission, requestPermission, hasPushSubscription, isSubscribing, sendTestPush } = useNotifications();
                ...
                {isOpen && (
                    <>
                        ...
                        >
                            ...
                            {/* Footer */}
                            <div className="p-3 border-t border-white/10 bg-black/20 text-center flex flex-col gap-3">
                                {permission === 'granted' && hasPushSubscription && (
                                    <button
                                        onClick={async () => {
                                            const success = await sendTestPush();
                                            if (success) {
                                                // Optional: close panel on success or show a toast
                                            }
                                        }}
                                        disabled={isSubscribing}
                                        className="text-xs bg-white/10 hover:bg-white/20 text-white font-bold py-2 px-4 rounded-lg border border-white/20 transition-all flex items-center justify-center gap-2"
                                    >
