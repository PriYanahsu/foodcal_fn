...
export const ExerciseList: React.FC<ExerciseListProps> = ({ selectedMuscle, searchQuery, difficulty }) => {
    ...
    const filteredExercises = EXERCISES.filter(ex => {
        ...
        return selectedMuscle ? matchesMuscle && finalMatches : (query.length > 2 || difficulty ? finalMatches : false);
    });
    ...
    if (!selectedMuscle && query.length <= 2 && !difficulty) {
        return (
            <div className="flex flex-col items-center justify-center py-20 md:py-32 text-center opacity-40">
                ...
                <h3 className="text-xl md:text-2xl font-black uppercase tracking-tight mb-2 text-white">Select a Muscle</h3>
                <p className="max-w-xs text-xs md:text-sm">Explore professional routines by selecting a focus area or using search.</p>
            </div>
        );
    }
                ...
                {!selectedExercise ? (
                    ...
                    >
                        <div className="flex items-center justify-between border-b border-white/5 pb-4">
                            <h3 className="text-lg md:text-xl font-black uppercase tracking-tight text-white">
                                ...
                                {query.length > 2 ? `Results for "${searchQuery}"` : selectedMuscle ? `${selectedMuscle} Routines` : 'Routines'}
                            </h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                            {filteredExercises.length > 0 ? (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <span className={`text-[9px] font-black uppercase tracking-widest ${ex.difficulty === 'beginner' ? 'text-green-400' :
                                                    ex.difficulty === 'intermediate' ? 'text-orange-400' : 'text-red-400'
                                                    }`}>
                                                    {ex.difficulty}
                                                </span>
                                            </div>
                                ...
                                <div className="col-span-full py-12 text-center bg-black/20 rounded-3xl border border-dashed border-[var(--card-border)]">
                                    <p className="text-[var(--text-muted)] italic text-sm md:text-base">No matching exercises found.</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
                    ...
                    >
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${selectedExercise.difficulty === 'beginner' ? 'text-green-400' :
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {selectedExercise.equipment.map((item, i) => (
                                        <span key={i} className="px-3 py-1.5 bg-white/5 rounded-lg border border-white/5 text-[10px] font-bold text-white uppercase">
                                            {item}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-4">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-white/30 whitespace-nowrap">Execution Protocol</h4>
                        ...
                        {selectedExercise.tips && selectedExercise.tips.length > 0 && (
                            <div className="p-5 rounded-2xl bg-[var(--primary)]/10 border border-[var(--primary)]/20 flex items-start gap-4">
                                ...
                                <div className="space-y-1">
                                    <p className="text-[10px] font-black text-[var(--primary)] uppercase tracking-widest">Training Pro Tip</p>
                                    <ul className="space-y-1">
                                        {selectedExercise.tips.map((tip, i) => (
                                            <li key={i} className="text-sm text-white font-medium">{tip}</li>
                                        ))}
                                    </ul>
                                </div>
                            </div>
