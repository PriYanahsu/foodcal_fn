...
export const StepTrackerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activityType, setActivityType] = useState<'walking' | 'running' | 'stationary'>(
    'stationary'
  );
  ...
  const updateStatisticalThreshold = (signalValue: number) => {
    ...
    // Calculate StdDev
    const variance =
      statsBufferRef.current.reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
      statsBufferRef.current.length;
  ...
  useEffect(() => {
    ...
    const handleMotion = (event: DeviceMotionEvent) => {
      ...
      gravityBaselineRef.current =
        alphaGrav * gravityBaselineRef.current + (1 - alphaGrav) * rawMag;
      ...
      smoothedSignalRef.current =
        alphaSmooth * smoothedSignalRef.current + (1 - alphaSmooth) * userForce;
  ...
  const savePendingToLocal = () => {
    if (!user) return;
    localStorage.setItem(`pending_steps_${user.id}`, JSON.stringify(pendingStepsRef.current));
  };
