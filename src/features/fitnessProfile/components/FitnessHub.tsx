          ...
          <div className="relative z-10 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                {
                  label: 'Weight',
                  value: fitnessProfile?.weight ? `${fitnessProfile.weight} kg` : '--',
                },
                {
                  label: 'Target',
                  value: fitnessProfile?.targetWeightKg
                    ? `${fitnessProfile.targetWeightKg} kg`
                    : '--',
                },
                { label: 'BMI', value: bmi },
                {
                  label: 'Height',
                  value: fitnessProfile?.height ? `${fitnessProfile.height} cm` : '--',
                },
                {
                  label: 'Activity',
                  value: fitnessProfile?.activityLevel?.replace('_', ' ') || '--',
                },
                {
                  label: 'Age',
                  value: fitnessProfile?.age ? `${fitnessProfile?.age ?? '--'}` : '--',
                },
              ].map((item) => (
                <div
                  key={item.label}
              ...
              {[
                ...
                >
                  ...
                  <p className="text-xs lg:text-sm font-black text-[var(--foreground)] tabular-nums">
                    {m.value ?? '--'}
                    <span className="text-[8px] font-medium text-[var(--text-muted)] ml-0.5">
                      {m.unit}
                    </span>
                  </p>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 pt-1 border-t border-[var(--card-border)]">
                {weightDelta && (
                  <span className="text-[10px] text-[var(--text-muted)]">
                    <span className="font-bold text-[var(--foreground)]">{weightDelta} kg</span> to
                    goal
                  </span>
                )}
                {daysLeft !== null && (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 lg:gap-3">
            <StatCard
              ...
              value={fitnessProfile?.dailyCalorieTarget || '--'}
              unit="kcal"
              icon="🔥"
              color="var(--primary)"
              delay={0}
            ...
            <StatCard
              label="Protein Goal"
              value={fitnessProfile?.dailyProteinTargetG || '--'}
              unit="g"
              icon="🥩"
              color="#2196f3"
              delay={0.05}
            ...
            <StatCard
              label="Carbs Goal"
              value={fitnessProfile?.dailyCarbsTargetG || '--'}
              unit="g"
              icon="🍞"
              color="#ff9800"
              delay={0.1}
            ...
            <StatCard
              label="Fats Goal"
              value={fitnessProfile?.dailyFatTargetG || '--'}
              unit="g"
              icon="🥑"
              color="#e91e63"
              delay={0.15}
                  ...
                  {[
                    {
                      ...
                      value: `${fitnessProfile?.weight || '--'} kg`,
                      icon: ScaleIcon,
                    },
                    {
                      label: 'Target Weight',
                      value: `${fitnessProfile?.targetWeightKg || '--'} kg`,
                      icon: TrophyIcon,
                    },
                    {
                      label: 'Height',
                      value: `${fitnessProfile?.height || '--'} cm`,
                      icon: BoltIcon,
                    },
                    { label: 'BMI', value: bmi, icon: HeartIcon },
                    {
                      label: 'Objective',
                      value: fitnessProfile?.objective || 'Not Set',
                      icon: SparklesIcon,
                    },
                    {
                      label: 'Activity',
