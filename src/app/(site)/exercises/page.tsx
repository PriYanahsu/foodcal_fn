        ...
        <div className="p-3 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 min-h-screen">
            ...
            <header className="flex flex-col gap-5">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                        ...
                        {selectedMuscle && (
                            <div className="flex items-center gap-2 bg-[var(--primary)]/10 px-4 py-3 rounded-xl border border-[var(--primary)]/20 shadow-lg w-full sm:w-auto">
                                <span className="text-[9px] font-black uppercase tracking-widest text-[var(--primary)] shrink-0">Target:</span>
                                <span className="text-[10px] font-black text-white uppercase truncate">{selectedMuscle}</span>
                                <button onClick={() => setSelectedMuscle(null)} className="ml-1 text-white/40 hover:text-white transition-colors">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-white/30 mr-1 sm:mr-2">Level:</p>
                        ...
                        {['beginner', 'intermediate', 'advanced'].map((level) => (
                            <button
                                ...
                                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-[0.2em] border transition-all ${selectedDifficulty === level
                                    ? level === 'beginner' ? 'bg-green-400 border-green-400 text-black shadow-[0_0_15px_-5px_#4ade80]' :
                                        level === 'intermediate' ? 'bg-orange-400 border-orange-400 text-black shadow-[0_0_15px_-5px_#fb923c]' :
