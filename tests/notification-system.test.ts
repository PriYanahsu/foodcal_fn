...
async function testNotificationSystem() {
  ...
  await runTest('Clear existing notifications', async () => {
    const { error } = await supabase.from('notifications').delete().eq('user_id', TEST_USER_ID);
  ...
  await runTest('Create incomplete diet scenario', async () => {
    ...
    if (error) throw error;
    console.log(
      `   Created food log: ${loggedCalories} calories (${Math.round((loggedCalories / targetCalories) * 100)}% of target)`
    );
  });
  ...
  await runTest('Verify duplicate prevention', async () => {
    ...
    if (duplicates.length > 0) {
      console.log(
        `   Note: Found duplicate notifications (this is expected in test mode):`,
        duplicates
      );
    }
